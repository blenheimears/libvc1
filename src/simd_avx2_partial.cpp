...
static inline void forward8_from_rows(const __m256d lo[8], const __m256d hi[8],
    ...
    for (int a=0;a<8;++a) {
        ...
        for (int y=0;y<8;++y) {
            ...
            if (use_fma) {
                accl=_mm256_add_pd(_mm256_mul_pd(k,lo[y]),accl);
                acch=_mm256_add_pd(_mm256_mul_pd(k,hi[y]),acch);
            } else {
    ...
    for (int a=0;a<8;++a) {
        ...
        for (int x=0;x<8;++x) {
            ...
            if (use_fma) {
                accl=_mm256_add_pd(_mm256_mul_pd(v,kl),accl);
                acch=_mm256_add_pd(_mm256_mul_pd(v,kh),acch);
            } else {
...
void add_block_rect_avx2_partial(uint8_t* dst,int stride,const int* block64,int width,int height) {
...
void luma_mc_block_avx2_partial(uint8_t* dst,int dst_stride,const uint8_t* ref_center,int ref_stride,
...
void luma_mc_block_avg_avx2_partial(uint8_t* dst,int dst_stride,const uint8_t* ref_center,int ref_stride,
                                 int width,int height,int hm,int vm,bool rnd,bool bilinear) {
    for (int y=0;y<height;++y) for (int x=0;x<width;x+=8) {
        const uint8_t* r=ref_center+static_cast<ptrdiff_t>(y)*ref_stride+x;
        const __m256i pred=bilinear?luma_pred8_bilinear(r,ref_stride,hm!=0,vm!=0,rnd)
...
void chroma_mc_block_avx2_partial(uint8_t* dst,int dst_stride,const uint8_t* ref_center,int ref_stride,
...
void chroma_mc_block_avg_avx2_partial(uint8_t* dst,int dst_stride,const uint8_t* ref_center,int ref_stride,
                                   int width,int height,int fx,int fy,bool rnd) {
    for (int y=0;y<height;++y) for (int x=0;x<width;x+=8)
...
void intensity_map_plane_avx2_partial(uint8_t* dst,const uint8_t* src,size_t count,int scale,int shift) {
    size_t i=0;
...
void intensity_map_chroma_plane_avx2_partial(uint8_t* dst,const uint8_t* src,size_t count,int scale) {
    size_t i=0;
...
void forward_transform_residual_rect_avx2_partial(const uint8_t* src,const uint8_t* pred,int stride,
    ...
    for (int a=0;a<height;++a) {
        for (int x=0;x<width;x+=4) {
            ...
            for (int y=0;y<height;++y) {
                ...
                acc=use_fma?_mm256_add_pd(_mm256_mul_pd(kv,r),acc):_mm256_add_pd(acc,_mm256_mul_pd(kv,r));
            }
    ...
    for (int a=0;a<height;++a) {
        for (int bb=0;bb<width;bb+=4) {
            ...
            for (int x=0;x<width;++x) {
                ...
                acc=use_fma?_mm256_add_pd(_mm256_mul_pd(tv,kv),acc):_mm256_add_pd(acc,_mm256_mul_pd(tv,kv));
            }
...
uint64_t satd4x4_avx2_partial(const uint8_t* cur,int cur_stride,const uint8_t* pred,int pred_stride){return detail::satd4x4_target(cur,cur_stride,pred,pred_stride);}


void dequant_coefficients_avx2_partial(const int* q,int* out,int count,int double_quant,int mquant,bool nonuniform){detail::dequant_coefficients_target(q,out,count,double_quant,mquant,nonuniform);}
void inverse_transform_rect_avx2_partial(int* block64,int type){detail::inverse_transform_rect_target(block64,type);}
void overlap_vertical8_avx2_partial(int* edge,int stride){detail::overlap_vertical8_target(edge,stride);}
void overlap_horizontal8_avx2_partial(int* edge,int stride){detail::overlap_horizontal8_target(edge,stride);}
bool loop_filter4_h_avx2_partial(uint8_t* p,int stride,int x,int y,int pq){return detail::loop_filter4_h_target(p,stride,x,y,pq);}
bool loop_filter4_v_avx2_partial(uint8_t* p,int stride,int x,int y,int pq){return detail::loop_filter4_v_target(p,stride,x,y,pq);}
