...
uint64_t block_sad_penryn(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int ss){uint64_t sad=0;for(int y=0;y<h;y+=ss)sad+=sad_row_sampled(cur+static_cast<ptrdiff_t>(y)*cs,ref+static_cast<ptrdiff_t>(y)*rs,w,ss);if(ss>1)sad*=static_cast<uint64_t>(ss*ss);return sad;}
uint64_t frame_sad_penryn(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int dx,int dy,int ss){uint64_t sad=0;for(int y=0;y<h;y+=ss){const int sy=std::clamp(y+dy,0,h-1);const uint8_t* a=cur+static_cast<ptrdiff_t>(y)*cs;const uint8_t* b=ref+static_cast<ptrdiff_t>(sy)*rs;int x=0;while(x<w&&x+dx<0){sad+=std::abs(int(a[x])-int(b[0]));x+=ss;}const int last=std::min(w-1,w-1-dx);if(x<=last){const int samples=(last-x)/ss+1;const int span=(samples-1)*ss+1;const int vecspan=(span/16)*16;if(vecspan>=16){sad+=sad_row_sampled(a+x,b+x+dx,vecspan,ss);x+=vecspan;}while(x<=last){sad+=std::abs(int(a[x])-int(b[x+dx]));x+=ss;}}for(;x<w;x+=ss)sad+=std::abs(int(a[x])-int(b[w-1]));}if(ss>1)sad*=static_cast<uint64_t>(ss*ss);return sad;}

void forward_transform_intra8_penryn(const uint8_t* src,int stride,double* coeff64,bool){alignas(16) double rows[8][8];for(int y=0;y<8;++y)for(int x=0;x<8;x+=2){__m128d v=_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),_mm_set1_pd(128.0));_mm_store_pd(rows[y]+x,v);}forward8(rows,coeff64);}
void forward_transform_residual8_penryn(const uint8_t* src,const uint8_t* pred,int stride,double* coeff64,bool){alignas(16) double rows[8][8];for(int y=0;y<8;++y)for(int x=0;x<8;x+=2){__m128d v=_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),load_u8x2_as_double(pred+static_cast<ptrdiff_t>(y)*stride+x));_mm_store_pd(rows[y]+x,v);}forward8(rows,coeff64);}

void inverse_transform_8x8_penryn(int* b){
