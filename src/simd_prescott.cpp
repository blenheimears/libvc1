...
static inline uint64_t sad_row_sampled(const uint8_t* a,const uint8_t* b,int n,int ss) {
    ...
    for (;x+16<=n;x+=16) {
        __m128i va=_mm_lddqu_si128(reinterpret_cast<const __m128i*>(a+x));
        __m128i vb=_mm_lddqu_si128(reinterpret_cast<const __m128i*>(b+x));
...
static inline __m128d load_u8x2_as_double(const uint8_t* p) {
    ...
    const __m128i w=_mm_unpacklo_epi8(b,z);
    const __m128i i=_mm_unpacklo_epi16(w,z);
    return _mm_cvtepi32_pd(i);
}
static inline void forward8(const double rows[8][8],double* coeff64) {
    alignas(16) double tmp[8][8]{};
    for (int a=0;a<8;++a) for (int x=0;x<8;x+=2) {
        __m128d acc=_mm_setzero_pd();
        for (int y=0;y<8;++y) acc=_mm_add_pd(acc,_mm_mul_pd(_mm_set1_pd(forward_transform::k8[a][y]),_mm_load_pd(rows[y]+x)));
    ...
    for (int a=0;a<8;++a) for (int bb=0;bb<8;bb+=2) {
        ...
        for (int x=0;x<8;++x) {
            ...
            acc=_mm_add_pd(acc,_mm_mul_pd(_mm_set1_pd(tmp[a][x]),k));
        }
...
static inline __m128i clamp_i32(__m128i v){
    const __m128i z=_mm_setzero_si128(), hi=_mm_set1_epi32(255);
    const __m128i neg=_mm_cmpgt_epi32(z,v);
    v=_mm_andnot_si128(neg,v);
    const __m128i over=_mm_cmpgt_epi32(v,hi);
    return _mm_or_si128(_mm_and_si128(over,hi),_mm_andnot_si128(over,v));
}
static inline __m128i load_u8x4_i32(const uint8_t* p){
    uint32_t q;std::memcpy(&q,p,4);
    const __m128i b=_mm_cvtsi32_si128(static_cast<int>(q)),z=_mm_setzero_si128();
...
uint64_t block_sad_prescott(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int ss){uint64_t sad=0;for(int y=0;y<h;y+=ss)sad+=sad_row_sampled(cur+static_cast<ptrdiff_t>(y)*cs,ref+static_cast<ptrdiff_t>(y)*rs,w,ss);if(ss>1)sad*=static_cast<uint64_t>(ss*ss);return sad;}
uint64_t frame_sad_prescott(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int dx,int dy,int ss){uint64_t sad=0;for(int y=0;y<h;y+=ss){const int sy=std::clamp(y+dy,0,h-1);const uint8_t* a=cur+static_cast<ptrdiff_t>(y)*cs;const uint8_t* b=ref+static_cast<ptrdiff_t>(sy)*rs;int x=0;while(x<w&&x+dx<0){sad+=std::abs(int(a[x])-int(b[0]));x+=ss;}const int last=std::min(w-1,w-1-dx);if(x<=last){const int samples=(last-x)/ss+1;const int span=(samples-1)*ss+1;const int vecspan=(span/16)*16;if(vecspan>=16){sad+=sad_row_sampled(a+x,b+x+dx,vecspan,ss);x+=vecspan;}while(x<=last){sad+=std::abs(int(a[x])-int(b[x+dx]));x+=ss;}}for(;x<w;x+=ss)sad+=std::abs(int(a[x])-int(b[w-1]));}if(ss>1)sad*=static_cast<uint64_t>(ss*ss);return sad;}

void forward_transform_intra8_prescott(const uint8_t* src,int stride,double* coeff64,bool){alignas(16) double rows[8][8];for(int y=0;y<8;++y)for(int x=0;x<8;x+=2){__m128d v=_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),_mm_set1_pd(128.0));_mm_store_pd(rows[y]+x,v);}forward8(rows,coeff64);}
void forward_transform_residual8_prescott(const uint8_t* src,const uint8_t* pred,int stride,double* coeff64,bool){alignas(16) double rows[8][8];for(int y=0;y<8;++y)for(int x=0;x<8;x+=2){__m128d v=_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),load_u8x2_as_double(pred+static_cast<ptrdiff_t>(y)*stride+x));_mm_store_pd(rows[y]+x,v);}forward8(rows,coeff64);}

void inverse_transform_8x8_prescott(int* b){
...
void put_block8_prescott(uint8_t* dst,int stride,const int* block,int bias){const __m128i bv=_mm_set1_epi32(bias);for(int y=0;y<8;++y)for(int x=0;x<8;x+=4){__m128i v=_mm_add_epi32(_mm_lddqu_si128(reinterpret_cast<const __m128i*>(block+y*8+x)),bv);store_u8x4(dst+static_cast<ptrdiff_t>(y)*stride+x,v);}}
void add_block_rect_prescott(uint8_t* dst,int stride,const int* block,int w,int h){for(int y=0;y<h;++y)for(int x=0;x<w;x+=4){__m128i p=load_u8x4_i32(dst+static_cast<ptrdiff_t>(y)*stride+x);__m128i r=_mm_lddqu_si128(reinterpret_cast<const __m128i*>(block+y*8+x));store_u8x4(dst+static_cast<ptrdiff_t>(y)*stride+x,_mm_add_epi32(p,r));}}

uint64_t block_sad_luma_qpel_prescott(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd){uint64_t sad=0;for(int y=0;y<h;++y)for(int x=0;x<w;++x)sad+=std::abs(int(cur[static_cast<ptrdiff_t>(y)*cs+x])-luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd));return sad;}
uint64_t block_sad_luma_bilinear_prescott(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,bool fx,bool fy,bool rnd){uint64_t sad=0;for(int y=0;y<h;++y)for(int x=0;x<w;++x)sad+=std::abs(int(cur[static_cast<ptrdiff_t>(y)*cs+x])-bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd));return sad;}
void luma_mc_block_prescott(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd,bool bilinear){for(int y=0;y<h;++y)for(int x=0;x<w;++x)dst[static_cast<ptrdiff_t>(y)*ds+x]=static_cast<uint8_t>(bilinear?bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm!=0,vm!=0,rnd):luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd));}
void luma_mc_block_avg_prescott(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd,bool bilinear){for(int y=0;y<h;++y)for(int x=0;x<w;++x){uint8_t& d=dst[static_cast<ptrdiff_t>(y)*ds+x];int p=bilinear?bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm!=0,vm!=0,rnd):luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd);d=static_cast<uint8_t>((int(d)+p+1)>>1);}}
void chroma_mc_block_prescott(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int fx,int fy,bool rnd){for(int y=0;y<h;++y)for(int x=0;x<w;++x)dst[static_cast<ptrdiff_t>(y)*ds+x]=static_cast<uint8_t>(chroma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd));}
void chroma_mc_block_avg_prescott(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int fx,int fy,bool rnd){for(int y=0;y<h;++y)for(int x=0;x<w;++x){uint8_t& d=dst[static_cast<ptrdiff_t>(y)*ds+x];int p=chroma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd);d=static_cast<uint8_t>((int(d)+p+1)>>1);}}

void intensity_map_plane_prescott(uint8_t* dst,const uint8_t* src,size_t n,int scale,int shift){for(size_t i=0;i<n;++i)dst[i]=static_cast<uint8_t>(clamp_u8((scale*int(src[i])+shift+32)>>6));}
void intensity_map_chroma_plane_prescott(uint8_t* dst,const uint8_t* src,size_t n,int scale){for(size_t i=0;i<n;++i)dst[i]=static_cast<uint8_t>(clamp_u8((scale*(int(src[i])-128)+128*64+32)>>6));}

void forward_transform_residual_rect_prescott(const uint8_t* src,const uint8_t* pred,int stride,int w,int h,double* coeff,bool){alignas(16) double rows[8][8]{};alignas(16) double tmp[8][8]{};for(int y=0;y<h;++y)for(int x=0;x<w;x+=2)_mm_store_pd(rows[y]+x,_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),load_u8x2_as_double(pred+static_cast<ptrdiff_t>(y)*stride+x)));for(int a=0;a<h;++a)for(int x=0;x<w;x+=2){__m128d acc=_mm_setzero_pd();for(int y=0;y<h;++y){double k=h==8?forward_transform::k8[a][y]:forward_transform::k4[a][y];acc=_mm_add_pd(acc,_mm_mul_pd(_mm_set1_pd(k),_mm_load_pd(rows[y]+x)));}_mm_store_pd(tmp[a]+x,acc);}std::fill(coeff,coeff+64,0.0);for(int a=0;a<h;++a)for(int bb=0;bb<w;bb+=2){__m128d acc=_mm_setzero_pd();for(int x=0;x<w;++x){__m128d k=w==8?_mm_setr_pd(forward_transform::k8[bb][x],forward_transform::k8[bb+1][x]):_mm_setr_pd(forward_transform::k4[bb][x],forward_transform::k4[bb+1][x]);acc=_mm_add_pd(acc,_mm_mul_pd(_mm_set1_pd(tmp[a][x]),k));}acc=_mm_mul_pd(acc,_mm_set1_pd(1024.0));_mm_storeu_pd(coeff+static_cast<size_t>(a)*8+bb,acc);}}

void quantize_coefficients_prescott(const double* coeff,int* out,int count,double qs,double off,int max_level){for(int i=0;i<count;++i){double c=coeff[i],a=std::abs(c);if(a==0){out[i]=0;continue;}int n=int(std::ceil((a-off)/qs-0.5));n=std::clamp(n,1,max_level);double e=a-(double(n)*qs+off);out[i]=(e*e<a*a)?(c<0?-n:n):0;}}

uint64_t sum_u8_prescott(const uint8_t* src,size_t count){uint64_t total=0;const __m128i z=_mm_setzero_si128();size_t i=0;for(;i+16<=count;i+=16)total+=hsum128_epi64(_mm_sad_epu8(_mm_lddqu_si128(reinterpret_cast<const __m128i*>(src+i)),z));for(;i<count;++i)total+=src[i];return total;}
