...
static inline void forward8(const double rows[8][8],double* coeff64,bool use_fma) {
    ...
    for (int a=0;a<8;++a) for (int x=0;x<8;x+=2) {
        ...
        for (int y=0;y<8;++y) {const __m128d k=_mm_set1_pd(forward_transform::k8[a][y]);const __m128d r=_mm_load_pd(rows[y]+x);acc=use_fma?_mm_fmadd_pd(k,r,acc):_mm_add_pd(acc,_mm_mul_pd(k,r));}
    ...
    for (int a=0;a<8;++a) for (int bb=0;bb<8;bb+=2) {
        ...
        for (int x=0;x<8;++x) {
            ...
            {const __m128d v=_mm_set1_pd(tmp[a][x]);acc=use_fma?_mm_fmadd_pd(v,k,acc):_mm_add_pd(acc,_mm_mul_pd(v,k));}
        }
...
void put_block8_piledriver(uint8_t* dst,int stride,const int* block,int bias){const __m128i bv=_mm_set1_epi32(bias);for(int y=0;y<8;++y)for(int x=0;x<8;x+=4){__m128i v=_mm_add_epi32(_mm_loadu_si128(reinterpret_cast<const __m128i*>(block+y*8+x)),bv);store_u8x4(dst+static_cast<ptrdiff_t>(y)*stride+x,v);}}
void add_block_rect_piledriver(uint8_t* dst,int stride,const int* block,int w,int h){for(int y=0;y<h;++y)for(int x=0;x<w;x+=4){__m128i p=load_u8x4_i32(dst+static_cast<ptrdiff_t>(y)*stride+x);__m128i r=_mm_loadu_si128(reinterpret_cast<const __m128i*>(block+y*8+x));store_u8x4(dst+static_cast<ptrdiff_t>(y)*stride+x,_mm_add_epi32(p,r));}}

uint64_t block_sad_luma_qpel_piledriver(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd){uint64_t sad=0;for(int y=0;y<h;++y)for(int x=0;x<w;++x)sad+=std::abs(int(cur[static_cast<ptrdiff_t>(y)*cs+x])-luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd));return sad;}
uint64_t block_sad_luma_bilinear_piledriver(const uint8_t* cur,int cs,const uint8_t* ref,int rs,int w,int h,bool fx,bool fy,bool rnd){uint64_t sad=0;for(int y=0;y<h;++y)for(int x=0;x<w;++x)sad+=std::abs(int(cur[static_cast<ptrdiff_t>(y)*cs+x])-bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd));return sad;}
void luma_mc_block_piledriver(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd,bool bilinear){for(int y=0;y<h;++y)for(int x=0;x<w;++x)dst[static_cast<ptrdiff_t>(y)*ds+x]=static_cast<uint8_t>(bilinear?bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm!=0,vm!=0,rnd):luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd));}
void luma_mc_block_avg_piledriver(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int hm,int vm,bool rnd,bool bilinear){for(int y=0;y<h;++y)for(int x=0;x<w;++x){uint8_t& d=dst[static_cast<ptrdiff_t>(y)*ds+x];int p=bilinear?bilinear_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm!=0,vm!=0,rnd):luma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,hm,vm,rnd);d=static_cast<uint8_t>((int(d)+p+1)>>1);}}
void chroma_mc_block_piledriver(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int fx,int fy,bool rnd){for(int y=0;y<h;++y)for(int x=0;x<w;++x)dst[static_cast<ptrdiff_t>(y)*ds+x]=static_cast<uint8_t>(chroma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd));}
void chroma_mc_block_avg_piledriver(uint8_t* dst,int ds,const uint8_t* ref,int rs,int w,int h,int fx,int fy,bool rnd){for(int y=0;y<h;++y)for(int x=0;x<w;++x){uint8_t& d=dst[static_cast<ptrdiff_t>(y)*ds+x];int p=chroma_pred_scalar(ref+static_cast<ptrdiff_t>(y)*rs+x,rs,fx,fy,rnd);d=static_cast<uint8_t>((int(d)+p+1)>>1);}}

void intensity_map_plane_piledriver(uint8_t* dst,const uint8_t* src,size_t n,int scale,int shift){size_t i=0;const __m128i sv=_mm_set1_epi32(scale),sh=_mm_set1_epi32(shift+32);for(;i+4<=n;i+=4){__m128i v=load_u8x4_i32(src+i);v=_mm_srai_epi32(_mm_add_epi32(_mm_mullo_epi32(v,sv),sh),6);store_u8x4(dst+i,v);}for(;i<n;++i)dst[i]=static_cast<uint8_t>(clamp_u8((scale*int(src[i])+shift+32)>>6));}
void intensity_map_chroma_plane_piledriver(uint8_t* dst,const uint8_t* src,size_t n,int scale){size_t i=0;const __m128i sv=_mm_set1_epi32(scale),bias=_mm_set1_epi32(128*64+32),c128=_mm_set1_epi32(128);for(;i+4<=n;i+=4){__m128i v=_mm_sub_epi32(load_u8x4_i32(src+i),c128);v=_mm_srai_epi32(_mm_add_epi32(_mm_mullo_epi32(v,sv),bias),6);store_u8x4(dst+i,v);}for(;i<n;++i)dst[i]=static_cast<uint8_t>(clamp_u8((scale*(int(src[i])-128)+128*64+32)>>6));}

void forward_transform_residual_rect_piledriver(const uint8_t* src,const uint8_t* pred,int stride,int w,int h,double* coeff,bool use_fma){alignas(16) double rows[8][8]{};alignas(16) double tmp[8][8]{};for(int y=0;y<h;++y)for(int x=0;x<w;x+=2)_mm_store_pd(rows[y]+x,_mm_sub_pd(load_u8x2_as_double(src+static_cast<ptrdiff_t>(y)*stride+x),load_u8x2_as_double(pred+static_cast<ptrdiff_t>(y)*stride+x)));for(int a=0;a<h;++a)for(int x=0;x<w;x+=2){__m128d acc=_mm_setzero_pd();for(int y=0;y<h;++y){double k=h==8?forward_transform::k8[a][y]:forward_transform::k4[a][y];{const __m128d kv=_mm_set1_pd(k);const __m128d r=_mm_load_pd(rows[y]+x);acc=use_fma?_mm_fmadd_pd(kv,r,acc):_mm_add_pd(acc,_mm_mul_pd(kv,r));}}_mm_store_pd(tmp[a]+x,acc);}std::fill(coeff,coeff+64,0.0);for(int a=0;a<h;++a)for(int bb=0;bb<w;bb+=2){__m128d acc=_mm_setzero_pd();for(int x=0;x<w;++x){__m128d k=w==8?_mm_setr_pd(forward_transform::k8[bb][x],forward_transform::k8[bb+1][x]):_mm_setr_pd(forward_transform::k4[bb][x],forward_transform::k4[bb+1][x]);{const __m128d v=_mm_set1_pd(tmp[a][x]);acc=use_fma?_mm_fmadd_pd(v,k,acc):_mm_add_pd(acc,_mm_mul_pd(v,k));}}acc=_mm_mul_pd(acc,_mm_set1_pd(1024.0));_mm_storeu_pd(coeff+static_cast<size_t>(a)*8+bb,acc);}}

void quantize_coefficients_piledriver(const double* coeff,int* out,int count,double qs,double off,int max_level){const __m128d signbit=_mm_set1_pd(-0.0),qv=_mm_set1_pd(qs),ov=_mm_set1_pd(off),one=_mm_set1_pd(1.0),mx=_mm_set1_pd(double(max_level)),zero=_mm_setzero_pd(),half=_mm_set1_pd(0.5);int i=0;for(;i+2<=count;i+=2){__m128d c=_mm_loadu_pd(coeff+i),a=_mm_andnot_pd(signbit,c);__m128d n=_mm_div_pd(_mm_sub_pd(a,ov),qv);n=_mm_ceil_pd(_mm_sub_pd(n,half));n=_mm_min_pd(mx,_mm_max_pd(one,n));__m128d recon=_mm_add_pd(_mm_mul_pd(n,qv),ov),e=_mm_sub_pd(a,recon),err=_mm_mul_pd(e,e),err0=_mm_mul_pd(a,a),keep=_mm_cmplt_pd(err,err0),neg=_mm_cmplt_pd(c,zero),signedn=_mm_blendv_pd(n,_mm_sub_pd(zero,n),neg),selected=_mm_blendv_pd(zero,signedn,keep);__m128i qi=_mm_cvttpd_epi32(selected);out[i]=_mm_cvtsi128_si32(qi);out[i+1]=_mm_extract_epi32(qi,1);}for(;i<count;++i){double c=coeff[i],a=std::abs(c);if(a==0){out[i]=0;continue;}int n=int(std::ceil((a-off)/qs-0.5));n=std::clamp(n,1,max_level);double e=a-(double(n)*qs+off);out[i]=(e*e<a*a)?(c<0?-n:n):0;}}

uint64_t sum_u8_piledriver(const uint8_t* src,size_t count){uint64_t total=0;const __m128i z=_mm_setzero_si128();size_t i=0;for(;i+16<=count;i+=16)total+=hsum128_epi64(_mm_sad_epu8(_mm_loadu_si128(reinterpret_cast<const __m128i*>(src+i)),z));for(;i<count;++i)total+=src[i];return total;}
